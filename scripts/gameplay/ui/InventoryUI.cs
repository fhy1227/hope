...
/// <summary>
/// 背包 + 装备栏 UI（按 I 键切换）
/// 布局在 scenes/gameplay/combat/inventory_ui.tscn，脚本只负责数据绑定与交互
/// </summary>
public partial class InventoryUI : Control
{
    ...
    private void OnItemSellRequested(string uid)
    {
        ...
        Combat.Instance?.Run?.AddGold(gold);
    ...
    private void OnSellLowRarityClicked()
    {
        ...
        Combat.Instance?.Run?.AddGold(totalGold);
    ...
    private void SetOpen(bool open)
    {
        if (open)
        {
            var canPauseByInventory =
                GameManager.Instance?.State == GameState.Combat
                && Combat.Instance?.Run?.State == CombatState.Playing;
            if (!Visible && canPauseByInventory)
            {
                GetTree().Paused = true;
                _pausedBySelf = true;
            }
        ...
        if (_pausedBySelf)
        {
            ...
            if (run?.Phase != RunPhase.Shop && run?.State == CombatState.Playing)
