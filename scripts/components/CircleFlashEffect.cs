...
public partial class CircleFlashEffect : Node2D
{
    ...
    private const float StartAlpha = 0.42f;
    private const float EndScale = 1.06f;

    private float _radius;
    private Color _baseColor = Colors.White;
    private float _alpha = StartAlpha;
    private float _scale = 0.94f;
    ...
    {
        var effects = Main.Instance?.World.Effects;
        if (effects == null)
        {
            return;
        }
        ...
        flash.Play(radius, color ?? new Color(0.75f, 0.9f, 1f));
    }
    ...
    public void Play(float radius, Color color)
    {
        ...
        tween.TweenMethod(Callable.From<float>(SetAlpha), StartAlpha, 0f, Duration)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Quad);
        tween.TweenMethod(Callable.From<float>(SetScale), 0.94f, EndScale, Duration)
            .SetEase(Tween.EaseType.Out)
            .SetTrans(Tween.TransitionType.Quad);
